...
pub(crate) fn is_authorized(
    ...
    for pattern_str in public_uri_patterns {
        ...
        if regex.is_match(&uri) {
            debug!(
                "Access granted for Public URI: {}, method: {}, origin: {}",
                uri, method, origin
            );
            Ok(())?
        }
    ...
    let username = parts
        .next()
        .ok_or_else(|| Unauthorized("Missing username in token".to_string()))?
        .to_string();
    let password = parts
        .next()
        .ok_or_else(|| Unauthorized("Missing password in token".to_string()))?
        .to_string();
