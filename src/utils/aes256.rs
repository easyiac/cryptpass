...
fn build_keys(correlation_id: Option<String>, key_iv_base64: &str) -> Result<([u8; 32], [u8; 16]), CryptPassError> {
    ...
    let key_decoded: Vec<u8> = BASE64_STANDARD.decode(key_base64.as_bytes()).map_err(|ex| {
        cryptpass_error!(BadRequest, "Error decoding key_base64", correlation_id.clone(), ex.to_string())
    })?;
    ...
    let iv_decoded: Vec<u8> = BASE64_STANDARD.decode(iv_base64.as_bytes()).map_err(|ex| {
        cryptpass_error!(BadRequest, "Error decoding iv_base64", correlation_id.clone(), ex.to_string())
    })?;
...
pub(super) fn encryption(
    ...
    let ct = Aes256CbcEnc::new(&key.into(), &iv.into())
        ...
        .map_err(|ex| cryptpass_error!(BadRequest, "Error encrypting", correlation_id, ex.to_string()))?;
...
pub(super) fn decryption(
    ...
    let pt = Aes256CbcDec::new(&key.into(), &iv.into())
        ...
        .map_err(|ex| cryptpass_error!(InternalServerError, "Error decrypting", correlation_id, ex.to_string()))?;
...
async fn test() {
    ...
    let enc = encryption(correlation_id.clone(), &key_base64, &plaintext).expect("Error encrypting");
    println!("Encrypted: value: {}", enc);
    assert_eq!(enc, encrypted_text);
    let dec = decryption(correlation_id, &key_base64, &enc).expect("Error decrypting");
